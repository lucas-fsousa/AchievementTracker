-- Core/Waypoint.lua
-- Define um waypoint para o local de uma conquista (quando a entrada curada traz
-- coords). Usa o waypoint NATIVO do Blizzard (seta na tela + pin no mapa, sem
-- dependencia) e, se o TomTom estiver instalado, tambem cria um waypoint nele.
-- Tudo protegido (nunca quebra a tela).
...
-- jogo. Necessario quando a coordenada curada so tem a ZONA (sem uiMapID).
local zoneIndex
    ...
    if TomTom and TomTom.AddWaypoint then
        ns.Safe.Call("set TomTom waypoint", function()
            TomTom:AddWaypoint(map, fx, fy, {
                title = title or "AchievementTracker", from = "AchievementTracker",
...
function ns.Waypoint.ToItem(item)
    ...
    local map = co.map or ns.Waypoint.MapForZone(co.zone or (e and e.zone))
    if not map then
        ns.Print("couldn't resolve the map for this achievement's zone (" ..
            tostring(co.zone or (e and e.zone) or "?") .. ").")
        return false
    end
    local label = (e.zone and (item.name .. " — " .. e.zone)) or item.name or "location"
...
-- A conquista pode receber waypoint? Tem coord com uiMapID, OU uma zona que o jogo
-- consegue resolver para um uiMapID.
function ns.Waypoint.CanRoute(item)
    local e = item and item.entry
    local co = e and e.coords
    if not (co and co.x and co.y) then return false end
    if co.map then return true end
    local zone = co.zone or e.zone
    return (zone and ns.Waypoint.MapForZone(zone) ~= nil) or false
end
