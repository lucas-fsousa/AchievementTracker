...
Drop the screenshots referenced by `README.md` / `README.pt-BR.md` / the CurseForge
description here. Use the **exact filenames** below so the links resolve, then uncomment
the image lines in the READMEs (search for `<!-- screenshot`).
...
- Use a character with **mixed progress** so the list shows a good spread of tiers and
  some near-complete achievements bubbling up.
- For `detail-panel.png`, pick a **meta-achievement** with unmet prerequisites so the
  **Requires:** list is visible.
- Crop tightly, keep a consistent width (~900px) for a clean README. PNG preferred.
- Avoid capturing personal account info if any is visible.

After adding the files, uncomment the image lines in `README.md` and `README.pt-BR.md`
(and paste the CurseForge description, which references the same images by raw URL).
